...
void Application_Jump_Check(void)
{
	bool JumpToApplication = false;
		...
		/* Enable pull-up on the PE2 pin so we can use it to select the mode */
		PORTE |= (1 << 2);
		Delay_MS(10);

		/* If PE2 is not jumpered to ground, start the user application instead */
		JumpToApplication = ((PINE & (1 << 2)) != 0);
